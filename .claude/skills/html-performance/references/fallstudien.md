...
|---|---|---|---|---|
| 2026-09-17 abends | mccain-digital | Werkstatt als Konsole (Fenster/unten/rechts, Griff, Glossar, Fragen an Claude über Vercel-Funktion) | Lighthouse mobil `/` 99 unverändert (alles lädt erst nach Klick), runtime.js 8,8 KB Brotli; `v5dev-check` um 20 Prüfungen erweitert, erster Lauf 2× rot (Konsole auf voller Höhe deckte das Klickziel; Aufräum-Prüfung gegen „leer“ statt vorher/nachher), danach ALL OK; Endpunkt-Unit-Test 16/16 | [](messen-methode.md#27-aufräum-prüfungen-vergleichen-vorher-mit-nachher-nicht-mit-leer), [](messen-methode.md#10-zeitabhängige-tore-nur-allein-laufen-lassen) |
...
| 2026-09-11/12 | mccain-digital | "Mobil 41" war eine Fehlmessung + Logo-Animationen gefunden | Score 45→63 (Messfehler), 63→69 nach Fix | messen-methode.md, [](rendern-animationen.md#15-den-wahren-verursacher-isoliert-messen-statt-der-naheliegenden-komponente-die-schuld-zu-geben) |
| 2026-09-02 bis 09-10 | mccain-digital | Der Award-Audit: Render-Pipeline und Kategorien A–D | SVG-Konsolenfehler 70/37→0/0, Elemente ~4.200→~2.200 | [](rendern-hauptthread.md#28-platzhalter-markup-gehört-in-ein-inertes-template-nie-in-einen-lebendigen-custom-element-teilbaum), messen-methode.md, [](rendern-animationen.md#2-will-changetransform-gezielt-einsetzen-nicht-pauschal) |
| 2026-09-01 | mccain-digital | Der Refresh: index.html wird die Startseite | Dither-Feld 68 Long Tasks/4s → 0 Long Tasks | [](rendern-canvas-webgl.md#12-vor-der-naheliegenden-zeichenoperation-die-tatsächliche-ursache-profilen), [](rendern-canvas-webgl.md#18-getboundingclientrect-liefert-bei-transformierten-elementen-die-hülle-nicht-die-echte-box), messen-methode.md |
...
| Wave 6 | 360 (company-portal) | Cold-Cache-Artefakt bei next start | LCP 9,4s (kalt) vs. 2,8s (warm) | messen-methode.md |
...
| — | 360 / whatever-recall-internal | Headless-Chromium rendert WebGL schwarz | 2 Projekte unabhängig betroffen | messen-hauptthread-observer.md |
...
**Ausgangslage:** Owner: „ok bitte restliche seite auf html bauen." Die Startseite lief seit dem 13.9. als v5 (Bauzeit-Render in 8 Breiten × 4 Zuständen, handgeschriebenes `home.js` mit `apply()`), die 20 Unterseiten noch als React-Seiten (eine Klassenkomponente je Seite, ~40 State-Felder, jeder Zustandswechsel ein voller Re-Render von 1.000–2.400 Knoten, 64–84 ms bei 4× CPU laut `tools/reactprof.mjs`).

**Befund:** Die 21 Klassen teilen 74 Methoden (63 byte-identisch) und haben je 2–15 eigene Widgets (Preisrechner, Vergleichsschieber, Crawler-Tabs, Auto-Zyklen). Die Vorlagensprache des Exports ist klein und kein JavaScript: `{{ pfad }}`, `{{ a == b }}`, `<sc-for>`, `<sc-if>`, `onClick="{{ fn }}"`, `ref`; alle Werte kommen aus `renderVals()`. Der Startseiten-Weg (Zustände vorrendern, `apply()` von Hand) scheitert am nicht aufzählbaren Zustandsraum; Widgets von Hand wären ein zweiter Nachbau.

**Maßnahme:** `v5/runtime.js` als Binder für die Vorlagensprache (Regel [react-nextjs 31](react-nextjs.md#31-wenn-die-zustände-nicht-aufzählbar-sind-die-vorlagensprache-binden-nicht-das-framework-ausliefern)): Vorlage inert im Dokument, `data-dc-tpl` am Knoten, Basislinie ohne Schreiben, nach `setState` nur Abweichungen, Knoten aus der Vorlage erzeugen/entfernen, Handler am Element, `React`-Shim, keine Schreibvorgänge in übersprungenen `content-visibility`-Blöcken (Nachholen beim Rendern). Build (`tools/v5build.mjs`) als Schleife über die Routentabelle, nur noch Breiten-Renders, ganze Klasse je Seite mit gezählten Patches. Start in drei Tasks geteilt (Regel [rendern-hauptthread 34](rendern-hauptthread.md#34-den-skript-start-in-tasks-unter-50-ms-teilen-mit-einem-yield-ohne-frame-wartezeit)).

**Ergebnis (17.9.2026, lokal, :8897):**

| | vorher | nachher |
| --- | ---: | ---: |
| Seiten ohne React zur Laufzeit | 1 | 21 |
| Funktionsprüfung (`v5probe --all`, Desktop + Telefon) | – | 21/21, 0 Konsolenfehler, 0 Schreibvorgänge des Binders beim Start |
| Kosten je Zustandswechsel | 64–84 ms (React, 4× CPU) | 1–3 ms (Desktop, ungedrosselt), 4 ms auf /marke/ |
| Startseite Telefon 4×: TBT | 29 ms | 21 ms (96 ms mit dem Start in einem Task) |
| Startseite Telefon 4×: LCP / CLS | 272 ms / 0 | 284 ms / 0 |
| HTML Startseite roh / gzip | 268 / 38 KB | 399 / 60 KB (Vorlage, Ids, deep-Inhalt) |

**Fallen:** Reacts Render hüllt die Vorlage in `div.sc-host` ohne Id (Binder muss darunter starten); die annotierte Vorlage ist DOM-serialisiert (`onclick`, nicht `onClick`); Tabellen-Tags sind als `sc-raw-*` verschlüsselt; Anker-Regexe brechen, wenn ein Attribut die Reihenfolge ändert (`<span data-pt` wurde `<span data-dc-tpl=… data-pt` — 112 Notizen wären auf jedem Telefon gerendert worden); dieselbe Schleife steht in zwei Methoden (`startGradient`, `startWave`), Patches müssen die Methode nennen; `<path d>` mit Array (Suchtreffer) wirft sechs Konsolenfehler je Suche; `cv-audit` misst vor `fonts.ready` und meldet den Schriftwechsel als Drift (95 px), nach `fonts.ready` −1 px.
...
**Lehre:** Vor jeder Ursachenanalyse die Messgrundlage selbst verifizieren (Kompression, Cache-Header, tatsächlich angesprochene Domain) — ein systematischer Messfehler kann jede folgende Optimierungsentscheidung in die falsche Richtung lenken. Viele kleine Endlos-Animationen in einem winzigen Element können mehr kosten als ein großflächiger Canvas-Effekt; den tatsächlichen Verursacher immer per isoliertem A/B messen, nicht der naheliegenden Komponente die Schuld geben. → messen-methode.md, [](rendern-animationen.md#15-den-wahren-verursacher-isoliert-messen-statt-der-naheliegenden-komponente-die-schuld-zu-geben), [](rendern-hauptthread.md#27-tausende-individuelle-inline-styles-kosten-tbt-auch-ganz-ohne-javascript), widerlegt.md#16-hydrateroot-läuft-erfolgreich-gegen-einen-nachträglich-gesetzten-dom-snapshot, widerlegt.md#11-lokal-gemessene-sehr-niedrige-scores-spiegeln-die-live-performance, widerlegt.md#14-die-eigene-4100-lighthouse-aussage-stimmt-für-den-aktuellen-build, widerlegt.md#20-der-webgl-pixelstrom-ist-hauptursache-der-hohen-tbt
...
**Lehre:** In Produktion cached die Vercel Edge CDN die optimierte Bildvariante ohnehin, der warme lokale Wert entspricht dem echten Prod-Steady-State. → messen-methode.md
...
**Lehre:** WebGL/Canvas-Rendering nie ausschließlich per Headless-CI-Screenshot verifizieren. → messen-hauptthread-observer.md, [[browser-verify]]
...
**Lehre:** `tsc`+Unit-Test-grün reicht für DOM-manipulierende Features nicht als Fertig-Kriterium — ein Browser-Runtime-Test gegen echtes Chromium muss die tatsächliche DOM-Wirkung verifizieren. → messen-methode.md, [[browser-verify]]
...
- Automatisierte Gates, die stillschweigend "PASS" melden, sind einer der teuersten Fehler im Werkzeugkasten: tote Server, umbenannte Selektoren, blinde Fehlerkanäle, ein Guard gegen einen längst behobenen Bug — jedes Mal hätte eine einfache Mindestmenge-an-Messpunkten-Prüfung gereicht. Dazu gehört auch die Aufräum-Prüfung gegen „leer“ statt gegen den Wert von vorher (17.9.2026, `--acc`): sie war grün, während das Werkzeug den Seitenzustand löschte.
- UI-Tore finden Browser-Mechanik, die man beim Bauen übersieht: `preventDefault()` auf `pointerdown` unterdrückt `dblclick` (Doppeltipp selbst messen, zwei `pointerdown` < 400 ms); ein Klick fokussiert, der Fokus scrollt einen kleinen Container nach, und ein „schließen bei Scroll“-Popover ist dann schon wieder weg (Popover mitführen statt schließen); ein fixiertes Panel auf voller Höhe deckt das Klickziel des nächsten Tests („subtree intercepts pointer events“) — Tore müssen den Zustand herstellen, den der Test braucht, statt ihn vom vorigen Schritt zu erben (alle 17.9.2026, mccain-digital Werkstatt).
...
- Eine Template-Engine kann Attribute still verschlucken: die Export-Laufzeit von mccain-digital (support.js) ließ jedes Boolean-Attribut ohne Wert fallen (`required`, `download`), während `autocomplete="…"` daneben durchkam – jedes gebaute Formular validierte nichts, monatelang unbemerkt, weil Quelle und Screenshot richtig aussahen (gefunden 17.9. nachts durch eine A11y-Prüfung gegen live). Prüfung gehört ins Tor: im gebauten DOM `input.required`/`form.checkValidity()` gegen die Quelle vergleichen, nicht nur Markup lesen. Fix an der Stelle der Baukette, die die Quelle einliest (Normalisierer), nicht in der Export-Datei, die der nächste Export überschreibt – und vorher klären, welche Datei wirklich Quelle ist (ein Ordner `archive/` war es nicht).
