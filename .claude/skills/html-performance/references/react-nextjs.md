...
- **Wenn die Zustände nicht aufzählbar sind: die Vorlagensprache binden, nicht das Framework ausliefern** — ein Binder schreibt nach setState nur Abweichungen, 1–3 ms statt 64–84 ms ([→](#31-wenn-die-zustände-nicht-aufzählbar-sind-die-vorlagensprache-binden-nicht-das-framework-ausliefern))
...
**Woran man es erkennt:** Seite lädt fehlerfrei (0 Konsolenfehler, alle Assets 200), aber kein Element reagiert — eine stille Inertheit, die reine Ladezeit-/Fehlerkanal-Checks nicht erkennen (siehe [messen-methode.md](messen-methode.md#fehlerkanäle-und-wächter-zuverlässigkeit)).
...
**Woran man es erkennt:** Zwei `<title>`, zwei `description`, zwei `og:url`, zwei JSON-LD-Graphen, zwei `canonical`-Links pro Seite — einer davon oft fehlerhaft (relativ statt absolut). Ein Audit gegen die reine Server-Antwort sieht das nicht; erst ein Audit gegen die HYDRIERTE Seite deckt es auf (siehe [messen-methode.md](messen-methode.md#fehlerkanäle-und-wächter-zuverlässigkeit)).
...
**Warum:** `content-visibility:auto` entlastet Layout und Paint für übersprungene Abschnitte (volle Mechanik und der LCP-Effekt stehen in [rendern-hauptthread.md](rendern-hauptthread.md#1-content-visibility-auto-auf-lange-off-screen-sektionen-anwenden)). Eine einzelne lange React-Hydration-Aufgabe ist dagegen reines Skript — content-visibility rührt Skriptauswertungskosten nicht an.
...
### 31. Wenn die Zustände nicht aufzählbar sind: die Vorlagensprache binden, nicht das Framework ausliefern
**Warum:** Regel 8 endet dort, wo der Zustandsraum einer Seite nicht mehr endlich ist (Preisrechner mit Optionen und Zählern, Schieber, Auto-Zyklen, Suche, Sprachwechsel). Ein Framework zur Laufzeit rendert dann bei jedem Zustandswechsel den ganzen Baum; Widgets von Hand nachzubauen ist ein zweiter, driftender Nachbau des Designs. Wenn die Vorlagensprache klein ist (Pfad-Ausdrücke, Schleifen, Bedingungen, Ereignis-Handler; alle Berechnung in einer Funktion wie `renderVals()`), ersetzt ein eigener Binder das Framework: das Bauzeit-Render bleibt das HTML für den ersten Paint, die Vorlage reist inert mit, jeder Knoten trägt seine Vorlagen-Id, und nach jedem Zustandswechsel werden nur die Ausdrücke ausgewertet, deren Wert sich geändert hat, geschrieben.
**Woran man es erkennt:** Die Seiten teilen eine Vorlagensprache mit wenigen Konstrukten; die Zustandsfelder sind zahlreich, aber jeder Ausdruck in der Vorlage ist ein Pfad in ein Werte-Objekt; das Framework bringt kein Splitting mehr, weil die Komponente eine einzige Wurzel ist.
**Fix:** (1) Vorlage inert ausliefern (`<template>`), Knoten-Ids im Markup lassen. (2) Beim Start Vorlage und DOM im Gleichschritt gehen und die Werte merken, ohne zu schreiben — die Basislinie kostet dann keinen Style-Recalc, und per Media Query verborgene Blöcke bleiben unangetastet. (3) Nach `setState` alle Ausdrücke auswerten, nur Abweichungen schreiben; fehlende Knoten aus der Vorlage erzeugen, weggefallene Blöcke entfernen. (4) Ereignis-Handler am Element, aufgelöst aus der letzten Auswertung. (5) Das `React`, das die Seitenlogik noch nennt, als Shim (`createElement` → leichte Knoten, vom Binder in DOM übersetzt). (6) In übersprungenen `content-visibility`-Blöcken nichts schreiben, beim Rendern nachholen.
**Beleg:** mccain-digital `v5/runtime.js` (17.9.2026), 21 Seiten mit 500–700 Ausdrücken, 841 Schleifen, 726 Bedingungen, 1.114 Handlern. Kosten je Zustandswechsel 1–3 ms (Desktop, ungedrosselt; /marke/ mit 2.400 Knoten 4 ms) gegen 64–84 ms React-Re-Render bei 4× CPU (`tools/reactprof.mjs`). Startseite Telefon 4× CPU alt → neu: TBT 29 → 21 ms, LCP 272 → 284 ms, CLS 0. Alles, was der Export kann, läuft auf jeder Seite (Mega-Menü, Suche, DE/EN, Formulare, Widgets), geprüft mit `tools/v5probe.mjs --all`. Fallstudie: [fallstudien.md](fallstudien.md#2026-09-17--mccain-digital--v5-alle-21-seiten-ohne-react-ein-binder-für-die-vorlagensprache) · Sicherheit: gemessen
**Gilt für:** React/SSR, static HTML, Design-Tool-Exporte mit eigener Vorlagensprache
