...
- **`contain-intrinsic-size` pro Sektion und Breite als Inhaltshöhe messen (stabiler Schlüssel, `cv-heights.mjs`), nie raten oder pauschal setzen.** ([→](#2-seitengerüst-und-sektionen))
...
- **WebGL im Worker (`OffscreenCanvas`) mit Start nach dem FCP-Eintrag oder bewusst vor FCP, nie per rAF aufgeschoben; DPR und Auflösung deckeln, Zeichnen pausiert außerhalb Sicht/Tab.** ([→](#5-canvas-und-webgl))
...
**`contain-intrinsic-size` pro Sektion und Breite messen, nie pauschal setzen.** Ein einziger geschätzter Wert für jede `[data-cv]`-Sektion (z. B. `contain-intrinsic-size: auto 900px` für alle) verzerrt die Dokumentlänge, bis jede Sektion einmal gerendert hat: gemessen an v5 lag die Seite direkt nach dem Laden am Desktop 3.095 px kürzer und mobil 3.402 px länger als nach einem vollständigen Scroll-Durchlauf — konkret sprang `#konfig-band` von 981 auf tatsächlich gerenderte 245 px, `div.s249` von 1.806 auf 2.274 px. Sprungmarken (`#anchor`-Links, „Zum Inhalt springen") landen in der Zwischenzeit daneben. CLS bleibt davon unberührt (der Sprung passiert unterhalb des sichtbaren Bereichs), aber die Schätzung ist trotzdem falsch. **Der korrekte Weg:** `contain-intrinsic-size` je Sektion **und** je Breite zur Bauzeit aus einem echten gerenderten Layout messen und als konkreten Pixelwert in den generierten CSS-Block schreiben — nicht als Autor-Schätzung im Quelltext stehen lassen. Gemessen wird die **Inhaltshöhe** (`clientHeight` minus vertikales Padding), denn `contain-intrinsic-size` beschreibt die Inhaltsbox; Padding und Rahmen legt das Layout obendrauf (mit der Rahmenbox gemessen startet jede gepolsterte Sektion exakt um ihr Padding zu groß, in v5 +72 px je Sektion). Jede Sektion braucht dafür einen **stabilen Schlüssel** (id oder data-Attribut, nie generierte Klassennamen), und die Messung wird nach jeder Inhalts- oder Layoutänderung wiederholt. **So gebaut in v5 (16.9.2026):** `tools/v5heights.mjs` misst nach einem vollständigen Scroll-Durchlauf bei 412/620/760/960/1280/1350 px und schreibt `tools/v5-heights.json`; `tools/v5build.mjs` stempelt `data-cv="fall"`, `"cv-2"` … und schreibt `[data-cv="key"]{contain-intrinsic-size:auto Hpx}` je Breite als `@media (min-width)`-Gruppen hinter die pauschale Regel. Ergebnis: Drift beim ersten Scroll mobil +3.402 → 0 px, Desktop −3.095 → −1 px. Generisch im Skill: `scripts/cv-heights.mjs <url> --widths …` liefert die fertigen Regeln, `scripts/cv-audit.mjs` meldet Drift über 24 px als Befund.
...
**Konsequenz: Canvas/WebGL läuft in einem Worker oder bleibt bewusst vor dem ersten Paint — nie „ein bisschen später“ auf dem Hauptthread.** Der einzige Weg, der den Hauptthread wirklich entlastet, ist `OffscreenCanvas` in einem Web Worker (Chromium, Firefox ab 105, Safari ab 17): `getContext`, Shader-Compile und Zeichnen laufen dort, und der Worker startet erst, wenn `first-contentful-paint` als Performance-Eintrag gemeldet ist (PerformanceObserver, `buffered: true`), weil der GPU-Prozess sonst die Präsentation des ersten Frames verzögert. Wer keinen Worker baut, lässt den Start bewusst vor FCP: das kostet FCP/LCP, aber keinen TBT, denn TBT zählt erst ab FCP. **Nicht** den Aufruf nur hinter ein oder zwei `requestAnimationFrame` oder hinter `load` schieben — gemessen (v5, 16.9.2026, SwiftShader mobil) blieb FCP bei 2,2 s, weil der GPU-Prozess den ersten Frame erst nach der Kontext-Erzeugung präsentiert, und der Hauptthread bekam stattdessen eine Long Task von 2,3 s nach FCP (TBT 2,2 s statt 0). Bis zum ersten Frame des Canvas zeigt CSS einen Fallback — ein Verlauf oder eine Farbe, kein Canvas-Element, das auf seinen Kontext wartet. Getestet wird das gezielt mit `scripts/fps.mjs --software-gl` (Chrome mit `--use-angle=swiftshader` gestartet) zusammen mit `scripts/lcp-window.mjs` (FCP-Zeile) — CPU-Drosselung allein in DevTools/Lighthouse prüft das **nicht**: sie throttelt den Hauptthread, nicht den GPU-Prozess, in dem WebGL-Shader laufen (**gemessen**: CPU-Throttling allein veränderte die Shader-Framerate nicht).
