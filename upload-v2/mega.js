...
(function () {
  ...
  function open() {
    // idempotent: a second open() (double trigger click, programmatic re-open)
    // must NOT re-capture lastFocus — activeElement is now inside the mega, so
    // close() would return focus to a mega link instead of the real trigger.
  ...
  function close() {
    // idempotent: guards the Esc + queued hash-link setTimeout(close) race so
    // focus isn't yanked back to the trigger after a hash navigation moved it.
    if (!mega.classList.contains("open")) return;
