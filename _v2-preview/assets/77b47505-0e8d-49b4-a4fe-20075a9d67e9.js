...
(function () {
  ...
  // mockup: accent re-tokened to the McCain brand yellow
  var ACC = "#F5C518", PAPER = "#151515";   // neutral — no warm/brown cast
  ...
  function rasterizeNode(el, w, h, cb, scale, fontOverride) {
    var S = scale || DPR;   // consumers drawing coarse fields pass a sub-1 scale — orders of magnitude cheaper
    var fontSrc = fontOverride || window.PixelFXFontCSS;
    var clone = el.cloneNode(true);
    inlineStyles(el, clone);
    // media never rasterizes reliably inside foreignObject (external src
    // refs can hang the SVG load and strand the callback) — hide it in
    // the clone and drop the refs; live photos re-assemble via
    // PixelFX.image instead. [data-px-skip] opts any element out.
    if (clone.querySelectorAll) {
      var kill = clone.querySelectorAll("img,video,canvas,[data-px-skip]");
      for (var kk = 0; kk < kill.length; kk++) {
        kill[kk].style.visibility = "hidden";
        kill[kk].removeAttribute("src");
        kill[kk].removeAttribute("srcset");
      }
    }
    // mockup: the site uses a webfont — a foreignObject image can't reach
    // document fonts, so the face is embedded as a data-URI @font-face
    // (window.PixelFXFontCSS, built at runtime in the bootstrap)
    var fontCSS = fontSrc ? "<style>" + fontSrc + "</style>" : "";
  ...
  function sampleField(img, imgW, w, h, gap, alphaMin, scale) {
    var S = scale || DPR;
    var parts = [];
  ...
  function headline(host) {
    ...
    // data-solid: the pixel assembly plays, then hands off to the REAL
    // crisp text (like the photos do). Hovering re-pixelates + black hole;
    // on leave it settles and swaps back to solid. Opt-in per host.
    var SOLID = host.dataset.solid !== undefined;
    ...
    // data-flash: assembly-sparkle colour — the global ACC (acid) vanishes
    // on light canvases and reads as dropped pixels, so light hosts pass
    // their own (e.g. the deep display green)
    var FLASH = host.dataset.flash || ACC;
    ...
    function frame(t) {
      ...
      for (var i = 0; i < parts.length; i++) {
        ...
        ctx.fillStyle = (p.flash && local < .82) ? FLASH : p.col;
    ...
    function settle() {
      ...
      interactive = true;
      if (SOLID) {
        // hand off to the crisp DOM text; the canvas only returns on hover
    ...
    function schedulePass(delay) {
      if (reduced || noHover || SOLID) return;   // solid headlines rest crisp — no roaming pass
    ...
    function physics() {
      ...
      else if (SOLID) { iraf = null; host.classList.remove("assembling"); cv.style.display = "none"; }
    ...
    // solid mode: the canvas is hidden at rest (pointer-events none), so the
    // hover interaction lives on the host — enter re-pixelates, leave settles
    // back and physics() swaps the crisp text in when the field parks
    if (SOLID && !noHover && !reduced) {
    ...
    if (!SOLID) velSubs.push(function (v) {
      hVel = v;
  ...
  function button(btn) {
    ...
    /* short accent flash at the gather point, then fire */
    function burst() {
  ...
  function voidReveal(host) {
    ...
    // data-ink: force every pixel to one colour — dark-on-light sampling
    // picks up washed anti-aliasing fringes otherwise (same fix as headlines)
    var VINK = host.dataset.ink || null;
    var VFLASH = host.dataset.flash || ACC;
    var DUR = 1500;
      ...
      function run() {
        ...
        function frame(t) {
          ...
          for (var i = 0; i < parts.length; i++) {
            ...
            ctx.fillStyle = (local < 0.75 && p.flash) ? VFLASH : p.col;
  ...
  function slides(stage) {
    ...
    var RS = 1 / 3;                    // bridge rasterizes at ⅓ resolution — the field is GAP-coarse anyway,
                                       // and full-DPR getImageData was blocking the main thread for seconds
    ...
    /* rasterize a slide's inner content into a coarse particle field,
       ...
       Fields are CACHED per slide (invalidated on resize) and can be
       pre-rasterized at idle via prime() — go() then starts instantly. */
    var fCache = new Map();
    function fieldOf(slideEl, cb) {
      var hit = fCache.get(slideEl);
      // key on stage WIDTH only: fields are top-left anchored, and goTo
      // itself mutates the stage's minHeight — a height-keyed cache can
      // never hit, so every nav re-rasterized both slides
      if (hit && Math.abs(hit.w - W) < 3) { cb(hit.parts); return; }
      ...
      rasterizeNode(inner, iw, ih, function (img, imgW) {
        if (!img) { cb([]); return; }
        var s = sampleField(img, imgW, iw, ih, GAP, 60, RS), out = [];
        for (var i = 0; i < s.length; i++) {
          out.push({ hx: s[i].hx + ox, hy: s[i].hy + oy, col: rgbStr(s[i].color) });
        }
        fCache.set(slideEl, { w: W, parts: out });
        cb(out);
      }, RS, window.PixelFXFontCSSLite);   // slides are sans-only — skip the big serif TTF embed
    }

    function prime(els) {
      var idle = window.requestIdleCallback
        ? function (fn) { requestIdleCallback(fn, { timeout: 1500 }); }
        : function (fn) { setTimeout(fn, 600); };
      idle(function () {
        size();
        (els || []).forEach(function (el) { fieldOf(el, function () { }); });
      });
    }

    addEventListener("resize", function () { fCache.clear(); });
    ...
    var gen = 0;
    function go(fromEl, toEl, dir, anim, done, onStart) {
      if (raf) { cancelAnimationFrame(raf); raf = null; }
      var myGen = ++gen;
      size();
      ...
      function start() {
        if (myGen !== gen) return;                 // cancelled while rasterizing
        if (onStart) onStart();                // fields ready — caller hides the real DOM now
        ...
        var t0 = null;
        function frame(t) {
          // NB: the local progress var below is `g` — the guard token must
          // NOT share that name (var hoisting made the old guard always bail)
          if (myGen !== gen) { raf = null; return; }
          if (!t0) t0 = t;
    ...
    function cancel() { gen++; if (raf) { cancelAnimationFrame(raf); raf = null; } ctx.clearRect(0, 0, W, H); }

    return { el: cv, go: go, cancel: cancel, resize: size, prime: prime, anims: ANIMS };
  }
