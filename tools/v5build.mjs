...
import { resolveColors } from "./tokens.mjs";
...
/* A colour is written in ONE place (tools/color_guard.mjs) - checked before anything is built. */
{
  const found = checkSources();
  if (found.length) throw new Error(`v5build: ${found.length} colour literal(s) outside tokens/colors.css\n  ` + found.slice(0, 12).join("\n  "));
}
...
function assemble(P, merged, heights) {
  ...
  if (/<h1 [^>]*data-hero-h1/.test(html)) {
    ...
    heroCss =
      ...
      resolveColors(`.v5-w{display:inline-block;color:var(--mc-action-text);background-image:var(--mc-heading-gradient);-webkit-background-clip:text;background-clip:text;-webkit-text-fill-color:transparent;padding-right:.04em;animation:wordIn .7s cubic-bezier(.2,.8,.2,1) both}`) +
...
/* THE TWO PAGES THAT ARE NOT ARTBOARDS. /404.html and /coming-soon/ are
 * written by hand. They used to live at their URLs with the token values
 * COPIED in ("copied rather than referenced") - and so they still showed the
 * old indigo when the brand moved to navy (19.9.2026). Their source is now
 * mccain-design-system/static/, with token references like every artboard,
 * and what ships is written here with the tokens resolved. */
for (const [from, to] of [["404.html", "404.html"], ["coming-soon.html", "coming-soon/index.html"]]) {
  const out = path.join(SITE, to);
  fs.mkdirSync(path.dirname(out), { recursive: true });
  fs.writeFileSync(out, resolveColors(fs.readFileSync(path.join(EXPORT_DIR, "static", from), "utf8")), "utf8");
}
...
fs.copyFileSync(path.join(import.meta.dirname, "motion-budget.js"), path.join(SITE, "werkstatt", "motion-budget.src.js"));
{
  ...
  const built = minify(resolveColors(fs.readFileSync(src, "utf8")), "GENERATED by tools/v5build.mjs from tools/runtime.js - edit the source, not this file");
...
{
  const found = checkOutputs();
  if (found.length) throw new Error(`v5build: ${found.length} unresolved token(s) in what ships\n  ` + found.slice(0, 12).join("\n  "));
  console.log("  colours: every token resolved (tools/color_guard.mjs)");
}
...
if (ONLY.length || failed || summary.length !== PAGES.length) {
  ...
  const cspHeader = JSON.parse(vercelText).headers.flatMap((h) => h.headers).find((h) => h.key === "Content-Security-Policy");
  ...
  if (cspChanged) {
    // Swap the one string in place: re-serialising the whole file would reflow
    // the hand-written redirect rules on every build that touches a script.
    const oldValue = JSON.stringify(cspHeader.value);
    if (vercelText.split(oldValue).length !== 2) throw new Error("v5build: the CSP value is not written exactly once in vercel.json - fill it in by hand once, the build then owns it");
    fs.writeFileSync(vercelPath, vercelText.replace(oldValue, () => JSON.stringify(CSP)), "utf8");
  }
