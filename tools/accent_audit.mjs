...
for (const p of PAGES) {
  for (const th of ["dark", "light"]) {
    const page = await openThemed(context, `${BASE}/${p}`, th);
    await settle(page);
    ...
    try {
      ...
      console.log(`${p} [${th}] PROBE THREW: ${e.message}`);
      fail = 1;
      await page.close();
      continue;
    }
    ...
    console.log(
      `${p} [${th}] accent nodes ${d.accentTextNodes}, pixel canvases ${d.canvasesChecked} ` +
...
console.log("\naccent text passes on every page, both themes (wordmark exempt by decision)");
